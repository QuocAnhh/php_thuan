...
function applications_store() {
    ...
    try {
        // Insert into applications table (only basic info based on current schema)
...
function applications_show() {
    ...
    // Add security check: ensure the user can only see their own application unless they are an admin
    $user_id = $_SESSION['user_id'];
...
function handle_my_application() {
    ...
    if ($application = mysqli_fetch_assoc($result)) {
        // User has an application, redirect to show it
        header('Location: ' . base_url('application/show?id=' . $application['id']));
        exit();
    } else {
        // User does not have an application, redirect to create one
...
function majors_info() {
    ...
    // Get all majors for display (public info)
...
function admission_results() {
    ...
    // Get user's application and results
    $user_id = $_SESSION['user_id'];
